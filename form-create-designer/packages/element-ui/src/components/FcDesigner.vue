      ...
      <el-container style="height: 100%" :key="locale && locale.name">
        ...
        <el-container class="_fc-m">
          <el-header class="_fc-m-tools" height="45">
            ...
            <div v-if="$slots.title" class="_fc-m-tools-c">
...
export default defineComponent({
  ...
  props: {
    ...
    /** 是否启用内置警告：当不小心关闭浏览器，自动识别操作变化提示用户是否保存 */
    disableCheckLeaveNotSave: Boolean,
  },
  emits: [
    "active",
    "create",
  ...
  setup(props) {
    ...
    const methods = {
      ...
      handleSave() {
        vm.emit("save", getFcDesignerData());
      },
      openPreview() {
      ...
      getData() {
        return getFcDesignerData(false)
      }
    };

    const getFcDesignerData = (toJson = true) => {
      const jsonValueResult = {
      ...
      if (toJson) {
        ...
        return {
          rule: JSON.parse(jsonValueResult.rule),
          options: JSON.parse(jsonValueResult.options),
        };
      }
    };
    ...
    const emitFcDesignerDataChange = debounce(() => {
      // Ignore when `running context` disposed
      if (disposed) {
        return;
      }

      vm.emit("update", (toJson) => getFcDesignerData(toJson));
    }, 0);

    /** 实现数据变化通知外部事件，主要参考save（参考handleSave）事件的数据关联的源头数据变化 */
    watch(
      () => [
        // From `getJson` 涉及的源数据
        data.dragForm.rule[0].children,

        // From `getOptionsJson` 涉及的源数据
        data.formOptions,
        data.inputForm.data,
      ],
      () => {
        emitFcDesignerDataChange();
      },
      {
        deep: true,
      }
  ...
  created() {
    ...
    if (this.disableCheckLeaveNotSave !== true) {
      // Show warning for dev when it has been.
      const prevOnbeforeunload = window.onbeforeunload;
      if (
        prevOnbeforeunload !== null &&
        prevOnbeforeunload !== this.onbeforeunloadHandler
      ) {
        console.debug(
          "[form-create-designer] [warn] override the `window.onbeforeunload` handle, please ensure the process"
        );
        console.debug(
          "[form-create-designer] [debug] prevOnbeforeunload: ",
          prevOnbeforeunload
        );
      }

      // Force to this handler
      window.onbeforeunload = this.onbeforeunloadHandler;
    } else {
      console.debug("[form-create-designer] [info] mute check saving...");
    }
  },
  methods: {
